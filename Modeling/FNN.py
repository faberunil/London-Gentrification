...
# Function to create sequences for FNN training
def create_sequences(data, n_steps):
    X, y = [], []
    for i in range(len(data) - n_steps):
        seq_x, seq_y = data[i:i+n_steps].flatten(), data[i+n_steps]
        X.append(seq_x)
        y.append(seq_y)
...
def forecast_to_2030(model, initial_data, scaler, feature_columns, n_steps, area, final_year=2030):
    current_data = initial_data[-n_steps:].flatten().reshape(1, -1)
    predictions = []
    last_known_year = summarized_df[summarized_df['Area'] == area]['Year'].max()
    years_to_predict = final_year - last_known_year

    for _ in range(years_to_predict):
        next_step = model.predict(current_data)
        predictions.append(next_step)
        current_data = np.append(current_data[:, len(feature_columns):], next_step).reshape(1, -1)
...
for area in summarized_df['Area'].unique():
    ...
    # Build the FNN model
    model = Sequential([
        Flatten(input_shape=(n_steps * len(feature_columns),)),
    ...
    predictions_df = forecast_to_2030(model, last_steps_data, scaler, feature_columns, n_steps, area)

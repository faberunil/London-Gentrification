...
# Function to forecast future values up to a specific year using a GRU model
...
for area in summarized_df['Area'].unique():
    ...
    # Define and configure the RNN model using GRU
    model = Sequential([
        GRU(50, activation='relu', return_sequences=True, input_shape=(n_steps, X_train.shape[2])),
        Dropout(0.2),
...
all_predictions.index.name = 'Year'

# Change fromat from XX/XX/20XX to 20XX
all_predictions.index = all_predictions.index.year

# save to csv
all_predictions.to_csv('Data/GRU_predictions.csv')